...
export default function PageReader() {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-2" data-page-reader-ignore>
            {expanded && (
                <div
                    className="glass rounded-2xl border border-[var(--glass-border)] p-4 shadow-xl min-w-[220px] max-w-[min(92vw,280px)]"
                    style={{ background: '#ffffff' }}
                >
                    <p className="text-xs font-semibold uppercase tracking-widest text-[var(--accent-color)] mb-3">Page reader</p>
                    <p className="text-xs text-[var(--text-secondary)] mb-3 leading-relaxed">
                        Site-wide: reads each page’s main content (not the nav or footer). Add{' '}
                        <code className="text-[10px] text-[var(--accent-color)]">data-page-reader-content</code> on a section to read only that block.
                    </p>
                    ...
                    <input
                        ...
                        className="w-full mb-3 accent-[var(--accent-color)]"
                    />
                    {enVoices.length > 0 && (
                        <>
                            ...
                            <select
                                ...
                                className="w-full mb-3 text-sm rounded-lg bg-white border border-slate-200 text-[var(--text-primary)] px-2 py-1.5 focus:outline-none focus:border-[var(--accent-color)]"
                            >
                                {enVoices.map((v) => (
                    ...
                    <div className="flex gap-2">
                        {state === 'idle' ? (
                            <button
                                ...
                                className="flex-1 px-3 py-2 text-xs font-bold uppercase tracking-wider rounded-lg bg-[var(--accent-color)] text-white hover:bg-[var(--accent-hover)] transition-colors"
                            >
                                Listen
                            </button>
                        ) : (
                            <>
                                <button
                                    type="button"
                                    onClick={togglePause}
                                    className="flex-1 px-3 py-2 text-xs font-bold uppercase tracking-wider rounded-lg border border-[var(--accent-color)] text-[var(--accent-color)] hover:bg-[var(--accent-color)]/10"
                                >
            ...
            <button
                ...
                className="flex items-center gap-2 rounded-full border border-slate-200/90 bg-white/95 backdrop-blur-md px-4 py-3 shadow-[0_4px_16px_rgba(15,23,42,0.08)] hover:border-[var(--accent-color)]/60 hover:shadow-[0_6px_20px_rgba(15,23,42,0.12)] transition-all text-sm font-semibold text-[var(--text-primary)]"
