"""Shared visual system for The Vendor Survival & Sales Guide."""
...
INK = HexColor("#14181C")
CHARCOAL = HexColor("#1A2332")
CHARCOAL_DEEP = HexColor("#10161F")
GREEN = HexColor("#1F6B4A")
GREEN_SOFT = HexColor("#E6F3EC")
GOLD = HexColor("#B8892D")
GOLD_SOFT = HexColor("#F7F0DC")
BRICK = HexColor("#B85C38")
SAND = HexColor("#F3EDE3")
...
def styles() -> StyleSheet1:
    ...
    s.add(ParagraphStyle(name="H1", fontName="Georgia-Bold", fontSize=21, leading=25, textColor=CHARCOAL, spaceBefore=0, spaceAfter=8))
    s.add(ParagraphStyle(name="H2", fontName="Georgia-Bold", fontSize=14, leading=18.5, textColor=CHARCOAL, spaceBefore=10, spaceAfter=6))
    s.add(ParagraphStyle(name="H3", fontName="SourceSans-Bold", fontSize=11.2, leading=15, textColor=GREEN, spaceBefore=8, spaceAfter=4))
    s.add(ParagraphStyle(name="Kicker", fontName="SourceSans-Semibold", fontSize=8.4, leading=11, textColor=BRICK, alignment=TA_LEFT, spaceAfter=3, tracking=1.2))
    ...
    s.add(ParagraphStyle(name="BoxTitle", fontName="SourceSans-Bold", fontSize=10, leading=13, textColor=CHARCOAL, spaceAfter=3))
    s.add(ParagraphStyle(name="BoxBody", fontName="SourceSans", fontSize=10, leading=14, textColor=INK, alignment=TA_LEFT))
    s.add(ParagraphStyle(name="ScriptLabel", fontName="SourceSans-Bold", fontSize=8, leading=11, textColor=GOLD, spaceAfter=2, tracking=0.8))
    s.add(ParagraphStyle(name="ScriptBody", fontName="Georgia-Italic", fontSize=11, leading=15.5, textColor=CHARCOAL, alignment=TA_LEFT))
    s.add(ParagraphStyle(name="CopyHead", fontName="SourceSans-Bold", fontSize=8, leading=10, textColor=WHITE, tracking=0.6))
    s.add(ParagraphStyle(name="CopyName", fontName="SourceSans-Semibold", fontSize=8, leading=10, textColor=GOLD_SOFT, alignment=TA_RIGHT))
    s.add(ParagraphStyle(name="CopyBody", fontName="SourceSans", fontSize=10.2, leading=14.4, textColor=INK, alignment=TA_LEFT))
    ...
    s.add(ParagraphStyle(name="Cell", fontName="SourceSans", fontSize=8.4, leading=11.4, textColor=INK))
    s.add(ParagraphStyle(name="CellBold", fontName="SourceSans-Bold", fontSize=8.4, leading=11.4, textColor=CHARCOAL))
    s.add(ParagraphStyle(name="CellHead", fontName="SourceSans-Bold", fontSize=7.8, leading=10.8, textColor=WHITE, alignment=TA_CENTER))
    s.add(ParagraphStyle(name="Prompt", fontName="SourceSans-Semibold", fontSize=10, leading=14, textColor=CHARCOAL, spaceAfter=3))
    return s
...
def draw_header_footer(canvas, doc, section: str) -> None:
    ...
    canvas.drawString(MARGIN_L, PAGE_H - 8.2 * mm, "THE VENDOR SURVIVAL & SALES GUIDE")
    ...
    canvas.drawString(MARGIN_L, 5.2 * mm, "Predictable daily sales")
    canvas.drawRightString(PAGE_W - MARGIN_R, 5.2 * mm, "Confident communication")
    canvas.restoreState()
...
def draw_cover_page(canvas, doc) -> None:
    ...
    canvas.setFillColor(GREEN)
    canvas.roundRect(28 * mm, PAGE_H - 32 * mm, 78 * mm, 8 * mm, 2 * mm, stroke=0, fill=1)
    canvas.setFillColor(WHITE)
    canvas.setFont("SourceSans-Semibold", 8)
    canvas.drawCentredString(28 * mm + 39 * mm, PAGE_H - 29.4 * mm, "VENDOR SURVIVAL SYSTEM")

    canvas.setFillColor(WHITE)
    canvas.setFont("Georgia-Bold", 32)
    canvas.drawString(28 * mm, PAGE_H - 56 * mm, "The Vendor")
    canvas.drawString(28 * mm, PAGE_H - 70 * mm, "Survival &")
    canvas.drawString(28 * mm, PAGE_H - 84 * mm, "Sales Guide")
    canvas.setFont("Georgia-Italic", 13)
    canvas.setFillColor(HexColor("#E8C4B0"))
    canvas.drawString(28 * mm, PAGE_H - 98 * mm, "Phone-first scripts for Nigerian online vendors")

    canvas.setStrokeColor(GOLD)
    canvas.setLineWidth(1.4)
    canvas.line(28 * mm, PAGE_H - 106 * mm, 82 * mm, PAGE_H - 106 * mm)

    canvas.setFillColor(HexColor("#F3E6D8"))
    canvas.setFont("SourceSans", 11.5)
    y = PAGE_H - 122 * mm
    for line in [
        "Last price. I will get back to you.",
        "TikTok views. Zero WhatsApp DMs.",
        "The fear of sounding desperate.",
        "",
        "This book gives you the words,",
        "the daily rhythm, and the close -",
        "without begging, without insulting.",
    ]:
        canvas.drawString(28 * mm, y, line)
        y -= 6.0 * mm

    canvas.setFillColor(GREEN)
    canvas.roundRect(28 * mm, 42 * mm, PAGE_W - 50 * mm, 30 * mm, 3 * mm, stroke=0, fill=1)
    canvas.setFillColor(WHITE)
    canvas.setFont("SourceSans-Bold", 10)
    canvas.drawString(34 * mm, 62 * mm, "THE OUTCOME")
    canvas.setFont("SourceSans", 10)
    canvas.drawString(34 * mm, 54 * mm, "Predictable daily sales. Confident communication.")
    canvas.drawString(34 * mm, 47 * mm, "Copy. Edit the blanks. Send. Track what paid.")

    canvas.setFillColor(HexColor("#E8C4B0"))
    canvas.setFont("SourceSans", 9)
    canvas.drawString(28 * mm, 26 * mm, "WhatsApp  |  TikTok  |  Status  |  Selar")
    canvas.drawString(28 * mm, 20 * mm, "Digital products, fashion, services, and general vendors")
    canvas.restoreState()
...
class CopyPaste(Flowable):
    """Phone-first copy block with a [COPY-PASTE] header."""

    def __init__(self, name: str, words: str, width: float, stylesheet: StyleSheet1):
        super().__init__()
        self.name = name
        self.words = words
        self.box_width = width
        self.s = stylesheet
        self._label = Paragraph("[COPY-PASTE]", stylesheet["CopyHead"])
        self._name = Paragraph(esc(name).upper(), stylesheet["CopyName"])
        html = esc(words).replace("\n", "<br/>")
        self._body = Paragraph(html, stylesheet["CopyBody"])
        self._h = 50

    def wrap(self, aw, ah):
        inner = self.box_width - 16
        _, lh = self._label.wrap(70, ah)
        _, nh = self._name.wrap(inner - 74, ah)
        _, bh = self._body.wrap(inner, ah)
        self._lh, self._nh, self._bh = lh, nh, bh
        self._h = 22 + bh + 12
        return self.box_width, self._h
    ...
    def draw(self):
        ...
        c.setLineWidth(0.7)
        c.roundRect(0, 0, self.box_width, self._h, 4, stroke=1, fill=1)
        c.setFillColor(CHARCOAL)
        c.roundRect(0, self._h - 18, self.box_width, 18, 4, stroke=0, fill=1)
        c.rect(0, self._h - 18, self.box_width, 8, stroke=0, fill=1)
        self._label.drawOn(c, 8, self._h - 14)
        self._name.drawOn(c, 80, self._h - 14)
        self._body.drawOn(c, 8, 8)
